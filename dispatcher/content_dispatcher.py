...
【原子提炼约束 - 必须严格遵守】
每条原子必须同时包含以下三个维度，缺一不可：

1. 认知 (Cognition) - 提供深度洞察、独特视角或反常识观点
2. 金句 (Golden Sentence) - 极具传播力、朗朗上口的总结性短句（20字内）
3. 行动 (Action) - 具体可执行、可操作的步骤或建议

【每条原子必须包含的字段】
1. type: "cognition"(认知) / "quote"(金句) / "action"(行动) / "case"(案例)
2. cognition: 深度洞察或独特视角（50-150字）
3. golden_sentence: 传播金句（20字内，要有冲击力）
4. action: 可执行建议或步骤（50-150字）
5. thinking_model: 思维模型（认知偏差/复利效应/人性本质/职场策略等）
6. emotional_resonance: 情感共鸣（焦虑/希望/认同/惊讶/好奇/赋能）
7. target_audience: 目标受众
8. tags: 标签数组（3-5个）
9. risk_level: 0(安全)/1(需语境)
10. shelf_life: "long"/"medium"
...
【输出格式】
只输出JSON数组，每条原子必须同时包含cognition/golden_sentence/action三个字段。
请以JSON数组格式输出："""
...
【微博约束 - 严格遵守】
- 篇幅：约300字
- 调性：侧重公共讨论、即时观点，适合长文分析
- 结构：开头钩子（痛点/反常识）→核心观点展开→金句收尾→互动引导
- 必须有话题标签(#话题名)
- 结尾引导评论互动
...
【小红书约束 - 严格遵守】
- 篇幅：约1000字
- 调性：视觉化文字，多使用Emoji、排版轻盈、语感具象且具有生活感
...
【今日头条约束 - 严格遵守】
- 篇幅：约1500字
- 调性：强调逻辑严密性、信息密度，采用正式且客观的深度报道风格
